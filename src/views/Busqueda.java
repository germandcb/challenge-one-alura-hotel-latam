...
public class Busqueda extends JFrame {
		...
		btnbuscar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(tbReservas.isVisible()) {
					limpiarTabla(tbReservas);
					listarReservaPorId();
				} else if (tbHuespedes.isVisible()) {
					limpiarTabla(tbHuespedes);	
					listarHuespedesPorNombre();
				}
	...
	protected void modificarHuesped() {
		if(tieneFilaElegida(tbHuespedes)) {
			JOptionPane.showMessageDialog(this, "Por favor, elije un registro de huesped");
			return;
		}
		
		Optional.ofNullable(modeloH.getValueAt(tbHuespedes.getSelectedColumnCount(),  tbHuespedes.getSelectedRow()))
		.ifPresentOrElse(fila -> {
			...
			var filaModificada = this.huespedController.modificar(
					nombre,
					apellido,
					fechaNacimiento,
					nacionalidad,
	...
	private void listarHuespedesPorNombre() {
		...
		String nombre = (String) txtBuscar.getText();
		
		List<Huesped> huesped = this.huespedController.listar(nombre);
	...
	private void modificarReserva() {
		...
		.ifPresentOrElse(fila -> {
			...
			var filaModificada = this.reservaController.modificar(
					fechaEntrada,
					fechaSalida,
					valor,
					formaPago,
	...
	private void listarReservas() {
		List<Reserva> reserva = this.reservaController.listar();
	...
	private void listarReservaPorId() {
		if(txtBuscar.getText().equals("")) {
			listarReservas();
			JOptionPane.showMessageDialog(this, "Por favor, Ingrese el id para buscar");
			return;
		}
		
		Integer id = Integer.parseInt(txtBuscar.getText()); 
		List<Reserva> reserva = this.reservaController.listar(id);
